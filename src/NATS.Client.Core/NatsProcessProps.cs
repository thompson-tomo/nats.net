...
public record NatsProcessProps : NatsMessagingProps
{
    ...
    internal NatsProcessProps(NatsSubject subject, int subscriptionId)
        : base(subject)
    {
    ...
    internal bool IsReplyToRequest(string inboxPrefix) => Subject.IsInbox(inboxPrefix) && Subject.ToString().Length < inboxPrefix.Length + 1 + 22 + 1 + 22;

    internal long SubjectNumber(string inboxPrefix)
    {
        if (Subject.IsInbox(inboxPrefix) && _subjectValue == 0)
        {
